...
Route::get('/services/{service}', [PublicContentController::class, 'service'])->whereIn('service', ['cutlery-restoration','hollowware-care','asset-condition-review','recurring-care-plans'])->name('services.show');
Route::get('/industries/{industry}', [PublicContentController::class, 'industry'])->whereIn('industry', ['hotels-resorts','restaurants-groups','catering-events','procurement-operations'])->name('industries.show');
...
Route::get('/health', fn (): JsonResponse => response()->json(['status'=>'ok','service'=>config('app.name','IProFixer'),'environment'=>app()->environment(),'locale'=>app()->getLocale()]))->name('health');
Route::get('/ready', function (): JsonResponse {
    try { DB::select('select 1'); } catch (Throwable $exception) { report($exception); return response()->json(['status'=>'not_ready','database'=>'unavailable'],503); }
    return response()->json(['status'=>'ready','database'=>'available']);
})->name('ready');
Route::post('/locale/{locale}', function (string $locale): RedirectResponse { abort_unless(in_array($locale,['en','ar'],true),404); session(['locale'=>$locale]); return back(); })->name('locale.update');
