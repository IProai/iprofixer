...
const img = (id, width) => `https://images.pexels.com/photos/${id}/pexels-photo-${id}.jpeg?auto=compress&cs=tinysrgb&fit=crop&w=${width}&q=58`;
const media = {
  hero: img('35972325', compact ? 760 : 1500),
  cutlery: img('20849333', compact ? 560 : 760),
  hollowware: img('5038943', compact ? 560 : 760),
  assessment: img('5532837', compact ? 560 : 760),
  recurring: img('11773871', compact ? 560 : 760),
  before: img('3829548', compact ? 680 : 1100),
  after: img('20849333', compact ? 680 : 1100),
};
...
if (pageHero instanceof HTMLElement) {
  const source = path.endsWith('/cutlery-restoration') ? media.cutlery
    : path.endsWith('/hollowware-care') ? media.hollowware
    : path.endsWith('/asset-condition-review') ? media.assessment
    : path.endsWith('/recurring-care-plans') ? media.recurring
    : media.hero;
...
if (main && document.querySelector('.hero-new')) {
  ...
  if (comparisonEl instanceof HTMLElement && range instanceof HTMLInputElement) {
    const setPosition = (value) => comparisonEl.style.setProperty('--position', `${Math.max(4, Math.min(96, Number(value)))}%`);
