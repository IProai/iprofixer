...
<main id="main">
    ...
    <section class="section services-showcase">
        <div class="container">
            ...
            <div class="service-stack">
                <a class="service-row" href="{{ route('services') }}"><span>01</span><div><h3>{{ $ar ? 'ترميم أدوات المائدة' : 'Cutlery restoration' }}</h3><p>{{ $ar ? 'عناية بالمظهر والسطح وفق الحالة الفعلية للقطع.' : 'Surface and presentation care based on actual condition.' }}</p></div><b>↗</b></a>
                <a class="service-row" href="{{ route('services') }}"><span>02</span><div><h3>{{ $ar ? 'العناية بالقطع المجوفة' : 'Hollowware care' }}</h3><p>{{ $ar ? 'معالجة مدروسة لقطع التقديم المستخدمة بكثافة.' : 'Structured care for high-use service pieces.' }}</p></div><b>↗</b></a>
                <a class="service-row" href="{{ route('services') }}"><span>03</span><div><h3>{{ $ar ? 'التقييم الفني للمخزون' : 'Asset condition review' }}</h3><p>{{ $ar ? 'فهم الحالة والكميات والأولوية قبل اعتماد النطاق.' : 'Condition, quantity and priority review before scope approval.' }}</p></div><b>↗</b></a>
                <a class="service-row" href="{{ route('services') }}"><span>04</span><div><h3>{{ $ar ? 'برامج العناية الدورية' : 'Recurring care plans' }}</h3><p>{{ $ar ? 'دورات خدمة مخططة للمنشآت ذات الاستخدام المتكرر.' : 'Planned service cycles for high-frequency operations.' }}</p></div><b>↗</b></a>
            </div>
        </div>
    </section>
    ...
    <section class="section dark-section">
        <div class="container split-feature">
            <div><p class="eyebrow light-eyebrow">{{ $ar ? 'مصمم للضيافة' : 'Built for hospitality' }}</p><h2>{{ $ar ? 'نخاطب فرق التشغيل، لا الزوار فقط.' : 'Designed for operating teams—not just for show.' }}</h2><p>{{ $ar ? 'الموقع الجديد يربط الخدمة بالقطاع والدور التشغيلي، حتى يصل العميل بسرعة إلى المسار المناسب بدلاً من تصفح صفحات عامة.' : 'The new website connects service, industry and operational role so buyers reach the right path quickly instead of browsing generic brochure pages.' }}</p></div>
            <div class="industry-grid"><a href="{{ route('industries') }}"><span>01</span><strong>{{ $ar ? 'الفنادق والمنتجعات' : 'Hotels & resorts' }}</strong></a><a href="{{ route('industries') }}"><span>02</span><strong>{{ $ar ? 'المطاعم والمجموعات' : 'Restaurants & groups' }}</strong></a><a href="{{ route('industries') }}"><span>03</span><strong>{{ $ar ? 'التموين والفعاليات' : 'Catering & events' }}</strong></a><a href="{{ route('industries') }}"><span>04</span><strong>{{ $ar ? 'المشتريات والتشغيل' : 'Procurement & operations' }}</strong></a></div>
        </div>
    </section>

    <section class="section assessment-section">
        <div class="container assessment-grid">
            ...
            <form class="quick-assessment" method="post" action="{{ route('rfq.store') }}">@csrf<input type="hidden" name="source_page" value="home"><label>{{ $ar ? 'الاسم' : 'Name' }}<input name="name" required></label><label>{{ $ar ? 'البريد الإلكتروني' : 'Email' }}<input type="email" name="email" required></label><label>{{ $ar ? 'المنشأة' : 'Property / company' }}<input name="company"></label><label>{{ $ar ? 'نوع الخدمة' : 'Service need' }}<select name="service"><option value="assessment">{{ $ar ? 'تقييم فني' : 'Technical assessment' }}</option><option value="cutlery">{{ $ar ? 'ترميم أدوات المائدة' : 'Cutlery restoration' }}</option><option value="hollowware">{{ $ar ? 'العناية بالقطع المجوفة' : 'Hollowware care' }}</option><option value="recurring">{{ $ar ? 'خطة عناية دورية' : 'Recurring care plan' }}</option></select></label><label class="full-field">{{ $ar ? 'ملاحظات مختصرة' : 'Brief notes' }}<textarea name="message" rows="4"></textarea></label><label class="consent full-field"><input type="checkbox" name="consent" value="1" required><span>{{ $ar ? 'أوافق على استخدام المعلومات للتواصل بخصوص هذا الطلب.' : 'I agree that this information may be used to contact me about this request.' }}</span></label><button class="button button-dark full-field" type="submit">{{ $ar ? 'إرسال طلب التقييم' : 'Send assessment request' }}</button></form>
        </div>
    </section>
</main>
